...
export function AdminSmcClient() {
  const [items, setItems] = useState<SmcDto[]>([]);
  ...
  const [occupationWithAddress, setOccupationWithAddress] = useState("");
  ...
  async function load() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/admin/smc", { cache: "no-store" });
      const data = (await res.json()) as SmcDto[] & { error?: string };
      if (!res.ok) throw new Error(data.error || "Failed to load SMC members");
      setItems(data);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Failed to load SMC members");
    } finally {
      setLoading(false);
    }
  }
  ...
  useEffect(() => {
    ...
    setFatherOrSpouseName(current.fatherOrSpouseName);
    setDesignation(current.designation);
    setOccupationWithAddress(current.occupationWithAddress);
    setResidentialAddress(current.residentialAddress);
  ...
  async function save() {
    ...
    try {
      ...
      const res = await fetch(isEdit ? `/api/admin/smc/${editingId}` : "/api/admin/smc", {
  ...
  async function remove(id: string) {
    if (!confirm("Delete this SMC member?")) return;
    setError(null);
    try {
      const res = await fetch(`/api/admin/smc/${id}`, { method: "DELETE" });
      const data = (await res.json().catch(() => ({}))) as { error?: string };
    ...
    <div className="grid gap-6 lg:grid-cols-3">
      <div className="rounded-2xl border border-slate-200 bg-white p-6">
        <div className="flex items-center justify-between gap-4">
          <div className="text-sm font-semibold text-slate-900">
            {editingId ? "Edit SMC member" : "New SMC member"}
          </div>
          {editingId ? (
            <button
              type="button"
        ...
        <div className="mt-4 space-y-3">
          ...
          <label className="block">
            ...
            <textarea
              value={occupationWithAddress}
              onChange={(e) => setOccupationWithAddress(e.target.value)}
              rows={3}
          ...
          <label className="block">
            ...
            <input
              ...
              placeholder="/gallery/governance/smc-1.jpeg"
              className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-slate-200"
            />
          </label>
          <label className="block">
          ...
          >
            {saving ? "Saving…" : editingId ? "Save changes" : "Add SMC member"}
          </button>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 lg:col-span-2">
        ...
        {loading ? (
          ...
          <div className="mt-4 text-sm text-slate-600">No SMC members yet.</div>
        ) : (
          <div className="mt-4 space-y-3">
            {items.map((m) => (
              <div
