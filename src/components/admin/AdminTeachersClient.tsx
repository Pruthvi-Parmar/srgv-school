...
export function AdminTeachersClient() {
  ...
  async function load() {
    ...
    try {
      ...
      const data = (await res.json()) as TeacherDto[] & { error?: string };
    ...
    <div className="grid gap-6 lg:grid-cols-3">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 lg:col-span-2">
        ...
        {loading ? (
          ...
          <div className="mt-4 text-sm text-slate-600">No teachers yet.</div>
        ) : (
          <div className="mt-4 space-y-3">
            {items.map((t) => (
              <div
              ...
              >
                <div>
                  ...
                  {t.designation ? (
                    <div className="mt-1 text-xs font-medium text-slate-600">{t.designation}</div>
                  ) : null}
                  {t.qualification ? (
                  ...
                  {t.photo ? (
                    <div className="mt-1 text-xs text-slate-500">{t.photo}</div>
                  ) : null}
                </div>
